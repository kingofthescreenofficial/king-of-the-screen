...
export async function POST(request: Request) {
  ...
  try {
    ...
    const submission = await createContentSubmission({
      nickname: required(form, "nickname"), tagline: required(form, "tagline"), linkUrl: typeof form.get("linkUrl") === "string" ? required(form, "linkUrl") : undefined,
      file, mediaMime, bytes, sourceHash: createHash("sha256").update(request.headers.get("x-forwarded-for") ?? "unknown").digest("hex"),
    });
    ...
    return NextResponse.json({ code, error: "Content review could not approve this submission." }, { status: status[code] ?? 400 });
  }
}
