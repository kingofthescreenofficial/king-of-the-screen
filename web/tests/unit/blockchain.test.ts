...
const intent: StoredPaymentIntent = { id: "intent-1", nonce: "nonce-1", buyerWallet: "buyer", treasuryAddress: "treasury", operationsVaultAddress: "operations-vault", treasuryLamports: 80, operationsVaultLamports: 20, createdAt: 1_000_000, expiresAt: 1_090_000 };
...
function validFixture(): SolanaPaymentFixture {
  return {
    ...
    transaction: { message: { accountKeys: ["buyer"], instructions: [
      ...
      { program: "system", parsed: { type: "transfer", info: { source: "buyer", destination: "operations-vault", lamports: 20 } } },
...
describe("exact Solana payment verifier", () => {
  ...
  it.each([
    ...
    ["missing operations vault transfer", (tx: SolanaPaymentFixture) => { tx.transaction.message.instructions[2].parsed!.info!.destination = "other"; }, "INVALID_OPERATIONS_VAULT_TRANSFER"],
