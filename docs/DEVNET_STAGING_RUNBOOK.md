...
4. Generate `STAGING_ACCESS_TOKEN` with 32 random bytes encoded as base64url.
5. Run `npm run ops:validate:local` from `web`. It must report that payments are disabled.
6. Start the app with `npm run dev` from `web`.
7. Open `http://localhost:3000/staging/access?token=[STAGING_ACCESS_TOKEN]` in a browser with Phantom set to Solana devnet.
