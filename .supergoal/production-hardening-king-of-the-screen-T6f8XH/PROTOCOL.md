...
All paths below are rooted at this run's artifact directory (`.supergoal/production-hardening-king-of-the-screen-T6f8XH`) — the concrete namespaced path is baked in when this file is copied at Stage 7, so two runs in the same working tree read and write entirely separate artifacts.

## The loop

Repeat until `SUPERGOAL_RUN_COMPLETE` is printed:

1. Read `.supergoal/production-hardening-king-of-the-screen-T6f8XH/STATE.md`. Find `Current phase: N`.
2. Read `.supergoal/production-hardening-king-of-the-screen-T6f8XH/phases/phase-N.md`. This is your full work spec.
...
2. Re-read `.supergoal/production-hardening-king-of-the-screen-T6f8XH/ROADMAP.md` and pull every phase's acceptance criteria fresh from the original plan.
...
2. Write `.supergoal/production-hardening-king-of-the-screen-T6f8XH/phases/audit-fix-<round>.md` — a focused fix spec that targets only the failing criteria. Forbid scope creep. Use the affected phases' original VERIFY as the success gate.
...
2. Append the probe to `.supergoal/production-hardening-king-of-the-screen-T6f8XH/STATE.md` failure log.
...
2. Write a focused **fix spec** at `.supergoal/production-hardening-king-of-the-screen-T6f8XH/phases/phase-N.fix.md`. The fix spec:
